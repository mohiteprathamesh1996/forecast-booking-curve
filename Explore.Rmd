...
nested_forecasts_insights$`2023-09-10__XXX-DXB`
```
...
for (
  dep_date in c(
    "2023-09-10", "2023-09-11", "2023-09-13"
    )
  ) {
  pb$tick()
    ...
    tryCatch(
      {
        ...
        walk_forward_results <- future_map_dfr(splits$splits, function(split) {
          ...
          tryCatch(
              {
                # First attempt with seasonality
                model_prophet_with_reg <- prophet_reg(
                  growth = "logistic",
                  season = "multiplicative",
                  logistic_cap = target_cap,
                ...
                message("Model fit successful with multiplicative seasonality.")
              },
              error = function(e) {
                message("Error encountered: ", e$message)
                message("Retrying model fit without seasonality...")
          
                # Retry without the season parameter
                model_prophet_with_reg <- prophet_reg(
                  growth = "logistic",
                  logistic_cap = target_cap,
                  changepoint_num = 1 # Less sensitivity to noise
                ...
                message("Model fit successful without seasonality.")
              }
            )
...
```{r}
queries_df %>%
  filter(id == paste("2023-09-10", "XXX-DXB", sep = "__")) %>%
  pull(analysis)
```
...
```{r}
saveRDS(object = nested_forecasts_insights, file = "ouptut/nested_forecasts_insights.rds")

saveRDS(object = queries_df, file = "ouptut/queries_df.rds")
...
readRDS(
  file = "output/nested_forecasts_insights.rds"
  )
```
