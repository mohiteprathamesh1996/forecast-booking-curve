...
```{r}
nested_forecasts_insights$"2023-03-10__DXB-XXX"$walk_forward_results_summary

nested_forecasts_insights$"2023-03-10__DXB-XXX"$dynamic_plot

cat(nested_forecasts_insights$"2023-03-10__DXB-XXX"$query)
```


```{r}
queries_list <- lapply(names(nested_forecasts_insights), function(id) {
...
# Function to check token count (placeholder function for actual implementation)
get_token_count <- function(query) {
  # Use a method to count tokens in the query (e.g., OpenAI tokenizer)
  return(nchar(query))  # Placeholder for token count, replace with real function
}

# Function to split queries into chunks
split_into_chunks <- function(query, max_tokens = 8192 - 1000) {
  # Ensure query is within the allowed token limit (adjust based on actual token count)
  token_count <- get_token_count(query)
  chunks <- list()
  
  while (token_count > max_tokens) {
    # Split the query into smaller parts if it exceeds the max token limit
    chunk <- substr(query, 1, max_tokens)
    chunks <- append(chunks, chunk)
    query <- substr(query, max_tokens + 1, nchar(query))  # Remove the chunk from the original query
    token_count <- get_token_count(query)
  }
  
  # Add any remaining part of the query as the last chunk
  if (nchar(query) > 0) {
...
# Function to send API request for a chunk of the query
send_api_request <- function(chunk) {
  # Send the query chunk to OpenAI's GPT-4 model
...
# Function to process a query in chunks and gather results
process_query_chunks <- function(query) {
  # Split the query into chunks
  chunks <- split_into_chunks(query)
  
  # Process each chunk and gather results in parallel
  chunk_results <- future_map(chunks, send_api_request)
  
  # Combine the results (you can adjust this to your preferred format)
  combined_results <- paste(chunk_results, collapse = " ")
  return(combined_results)
}

# Run the query processing in parallel
results <- future_map(queries_df$query, process_query_chunks)

# Store the results (for further analysis or use)
results
```
...
# Example: Adding unique dataframes identified by (category, type) pairs
categories <- c("A", "B")
types <- c("X", "Y")

for (cat in categories) {
  for (typ in types) {
    # Create nested lists by category and type
    unique_items[[cat]][[typ]] <- data.frame(
      category = cat,
      type = typ,
      x = rnorm(3),
      y = rnorm(3)
...
saveRDS(unique_items, "unique_items_nested.rds")

# Load the stored RDS file
loaded_data <- readRDS("unique_items_nested.rds")

# Accessing specific datasets
loaded_data[["A"]][["X"]]  # Retrieve dataset for category A, type X
loaded_data[["B"]][["Y"]]  # Retrieve dataset for category B, type Y
```
